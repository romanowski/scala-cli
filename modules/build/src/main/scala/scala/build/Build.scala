...
object Build {
  ...
  private def copyResourceToClassesDir(build: Build) = build match {
    case b: Build.Successful =>
      for {
        ...
        if !relativeResourcePath.startsWith(os.rel / Inputs.buildDirName)
      } {
  ...
  def classesRootDir(root: os.Path, projectName: String): os.Path =
    root / Inputs.buildDirName / projectName / "classes"
  ...
  def buildProject(
    ...
    val project = Project(
      directory = inputs.workspace / Inputs.buildDirName,
  ...
  private def jmhBuild(
    ...
    val jmhOutputDir   = inputs.workspace / Inputs.buildDirName / jmhProjectName
